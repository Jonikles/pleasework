...
function initializeModals() {
    profileEditModal = new bootstrap.Modal(document.getElementById('profileEditModal'));
    addFundsModal = new bootstrap.Modal(document.getElementById('addFundsModal'));
    createBookingModal = new bootstrap.Modal(document.getElementById('createBookingModal'));
    reviewModal = new bootstrap.Modal(document.getElementById('reviewModal'));
    tutorDetailsModal = new bootstrap.Modal(document.getElementById('tutorDetailsModal'));
}
...
function initializeEventListeners() {
    ...
    document.getElementById('signupUserType').addEventListener('change', toggleTutorFields);
    ...
    document.getElementById('editStudentProfileBtn').addEventListener('click', showEditProfileModal);
    document.getElementById('findTutorBtn').addEventListener('click', showSearchTutors);
    document.getElementById('viewStudentBookingsBtn').addEventListener('click', showStudentBookings);
    document.getElementById('confirmAddFundsBtn').addEventListener('click', handleAddFunds);

    // Tutor dashboard listeners
    document.getElementById('editTutorProfileBtn').addEventListener('click', showEditProfileModal);
    document.getElementById('manageSubjectsBtn').addEventListener('click', showSubjectManagement);
    document.getElementById('manageAvailabilityBtn').addEventListener('click', showAvailabilityManagement);
    document.getElementById('viewTutorBookingsBtn').addEventListener('click', showTutorBookings);

    // Search listeners
    document.getElementById('backFromSearchBtn').addEventListener('click', backToDashboard);
    document.getElementById('searchForm').addEventListener('submit', handleSearch);

    // Booking management listeners
    document.getElementById('backFromBookingsBtn').addEventListener('click', backToDashboard);
    document.getElementById('upcomingTab').addEventListener('click', () => showBookingTab('upcoming'));
    document.getElementById('pastTab').addEventListener('click', () => showBookingTab('past'));
    document.getElementById('cancelledTab').addEventListener('click', () => showBookingTab('cancelled'));

    // Subject management listeners
    document.getElementById('backFromSubjectsBtn').addEventListener('click', backToDashboard);

    // Availability management listeners
    document.getElementById('backFromAvailabilityBtn').addEventListener('click', backToDashboard);
    ...
    document.getElementById('confirmBookingBtn').addEventListener('click', handleCreateBooking);
    document.getElementById('submitReviewBtn').addEventListener('click', handleSubmitReview);
    document.getElementById('bookingDuration').addEventListener('input', updateBookingCost);
    ...
    document.getElementById('availableSubjectsList').addEventListener('click', (e) => {
...
function showDashboard() {
    ...
    if (state.currentUser.userType === 'Student') {
        document.getElementById('balanceDisplay').textContent = `Balance: ${state.currentUser.balance}`;
        showStudentDashboard();
    } else {
        document.getElementById('balanceDisplay').textContent = `Rate: ${state.currentUser.hourlyRate}/hr`;
        showTutorDashboard();
    }
}
...
function hideAllViews() {
    const views = ['authView', 'studentDashboard', 'tutorDashboard', 'searchTutorsView', 
                   'bookingManagementView', 'subjectManagementView', 'availabilityManagementView'];
...
async function handleSignup(e) {
    ...
    const signupData = {
        ...
        hourlyRate: parseFloat(document.getElementById('signupHourlyRate').value) || 0,
...
async function loadStudentDashboard() {
    try {
        const response = await fetch(`${API_BASE_URL}/dashboard/student/${state.currentUser.id}`);
        if (response.ok) {
            const dashboard = await response.json();
            
            // Update profile
            document.getElementById('studentName').textContent = dashboard.profile.name;
            document.getElementById('studentBalance').textContent = dashboard.profile.balance.toFixed(2);
            
            // Update stats
            document.getElementById('totalSessions').textContent = dashboard.stats.totalSessions;
            document.getElementById('completedSessions').textContent = dashboard.stats.completedSessions;
            document.getElementById('upcomingSessions').textContent = dashboard.stats.upcomingSessions;
            
            // Update upcoming bookings
            renderUpcomingBookings(dashboard.upcomingBookings, 'upcomingBookingsList');
        }
    } catch (error) {
...
async function loadTutorDashboard() {
    try {
        const response = await fetch(`${API_BASE_URL}/dashboard/tutor/${state.currentUser.id}`);
        if (response.ok) {
            const dashboard = await response.json();
            
            // Update profile
            document.getElementById('tutorName').textContent = dashboard.profile.name;
            document.getElementById('tutorRate').textContent = dashboard.profile.hourlyRate.toFixed(2);
            
            // Update stats
            document.getElementById('tutorTotalSessions').textContent = dashboard.stats.totalSessions;
            document.getElementById('tutorCompletedSessions').textContent = dashboard.stats.completedSessions;
            document.getElementById('tutorRating').innerHTML = displayRating(dashboard.stats.averageRating);
            document.getElementById('tutorEarnings').textContent = dashboard.stats.totalEarnings.toFixed(2);
            document.getElementById('thisMonthEarnings').textContent = dashboard.stats.thisMonthEarnings.toFixed(2);
            document.getElementById('totalReviews').textContent = dashboard.stats.totalReviews;
...
function renderSearchResults(tutors) {
    ...
    if (tutors.length === 0) {
        container.innerHTML = '<p class="text-center">No tutors found matching your criteria.</p>';
        return;
    }
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <p>Rate: ${tutor.hourlyRate}/hr | Rating: ${tutor.rating.toFixed(1)}⭐ (${tutor.reviewCount} reviews)</p>
                        <p>Subjects: ${tutor.subjects.map(s => s.name).join(', ')}</p>
                    </div>
                    <div class="col-md-4 text-end">
                        <button class="btn btn-primary" onclick="bookTutor('${tutor.id}', '${tutor.name}', ${tutor.hourlyRate})">Book Session</button>
                        <button class="btn btn-secondary" onclick="viewTutorDetails('${tutor.id}')">View Profile</button>
                    </div>
                </div>
            </div>
        `;
...
async function showBookingTab(tab) {
    ...
    const endpoint = state.currentView === 'studentBookings' 
    ...
    try {
        ...
        if (response.ok) {
            const data = await response.json();
            const bookings = tab === 'upcoming' ? data.upcomingBookings :
                           tab === 'past' ? data.pastBookings : data.cancelledBookings;
            
            renderBookings(bookings, tab);
            
            // Update tab active states
            document.querySelectorAll('#bookingManagementView .nav-link').forEach(link => {
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <p>With: ${state.currentView === 'studentBookings' ? booking.tutor.name : booking.student.name}</p>
                        <p>Date: ${dateTime} | Duration: ${booking.durationHours} hour(s)</p>
                        <p>Total Cost: ${booking.totalCost} | Status: ${booking.status}</p>
                    </div>
                    <div class="col-md-4 text-end">
...
function getBookingActions(booking, tab) {
    ...
    } else if (tab === 'past' && booking.status === 'COMPLETED' && state.currentView === 'studentBookings') {
...
function renderUpcomingBookings(bookings, containerId) {
    ...
    bookings.forEach(booking => {
        ...
        div.innerHTML = `
            ...
            With: ${state.currentUser.userType === 'Student' ? booking.tutor.name : booking.student.name}<br>
            ${dateTime}
        `;
...
async function loadTutorSubjects() {
    try {
        ...
        const tutorResponse = await fetch(`${API_BASE_URL}/tutors/${state.currentUser.id}`);
        if (tutorResponse.ok) {
        ...
        const availableResponse = await fetch(`${API_BASE_URL}/subjects/available/tutor/${state.currentUser.id}`);
...
function renderAvailableSubjects(subjects) {
    ...
    if (subjects.length === 0) {
        container.innerHTML = '<p class="text-muted">All subjects already added.</p>';
        return;
    }
...
async function addSubject(subjectId) {
    try {
        const response = await fetch(`${API_BASE_URL}/tutors/${state.currentUser.id}/subjects`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function removeSubject(subjectId) {
    try {
        const response = await fetch(`${API_BASE_URL}/tutors/${state.currentUser.id}/subjects/${subjectId}`, {
            method: 'DELETE'
        });
...
async function loadTutorAvailability() {
    try {
        const response = await fetch(`${API_BASE_URL}/tutors/${state.currentUser.id}/availability`);
        if (response.ok) {
...
function renderAvailability(slots) {
    ...
    slots.sort((a, b) => dayOrder.indexOf(a.dayOfWeek) - dayOrder.indexOf(b.dayOfWeek));
...
async function handleUpdateProfile() {
    const updateData = {
        ...
        currentPassword: document.getElementById('editCurrentPassword').value || null,
    ...
    if (state.currentUser.userType === 'Tutor') {
        updateData.hourlyRate = parseFloat(document.getElementById('editHourlyRate').value) || null;
        updateData.description = document.getElementById('editDescription').value || null;
    }
    
    const endpoint = state.currentUser.userType === 'Student' 
...
async function handleAddFunds() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/students/${state.currentUser.id}/add-funds`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function updateBookingCost() {
    ...
    const hourlyRate = parseFloat(document.getElementById('bookingSubject').dataset.hourlyRate) || 0;
    const totalCost = duration * hourlyRate;
    document.getElementById('bookingTotalCost').textContent = totalCost.toFixed(2);
}
...
async function handleCreateBooking() {
    const bookingData = {
        ...
        dateTime: `${document.getElementById('bookingDate').value}T${document.getElementById('bookingTime').value}:00`,
    ...
    try {
        ...
        if (response.ok) {
            ...
            alert('Booking created successfully! Please confirm and pay to secure your session.');
            showStudentBookings();
        } else {
            const error = await response.text();
    ...
    try {
        ...
        if (response.ok) {
            ...
            content.innerHTML = `
                ...
                <p><strong>Rating:</strong> ${tutor.rating.toFixed(1)}⭐ (${tutor.totalReviews} reviews)</p>
                <p><strong>Description:</strong> ${tutor.description}</p>
                <p><strong>Subjects:</strong> ${tutor.subjects.map(s => s.name).join(', ')}</p>
                <p><strong>Completed Sessions:</strong> ${tutor.completedSessions}</p>
                <p><strong>Member Since:</strong> ${new Date(tutor.joinedDate).toLocaleDateString()}</p>
                <hr>
                <h5>Reviews</h5>
        ...
        if (response.ok) {
            ...
            container.innerHTML = reviews.map(review => `
                <div class="border-bottom pb-2 mb-2">
                    <strong>${review.studentInfo.name}</strong> - ${review.rating}⭐
